...
class Publisher():
    ...
    def publish(self, title:str, message: str, url:str) -> bool:
        """Publie un message sur Discord via le webhook."""
        try:
            response = self._publish_to_discord(self._discord_webhook,title, message,url)
    ...
    def _publish_to_discord(self, webhook: str, title:str, message: str, url:str) -> requests.Response:
        """Envoie le message au webhook Discord."""
        data = {"embeds":[{"image":{"url":url}},{"title":title, "description": message}]}
